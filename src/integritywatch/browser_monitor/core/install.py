...
from integritywatch.utils.browser import get_all_browser_native_host_paths, install_native_host_manifest_windows, get_installed_chromium_browsers
from integritywatch.utils.logger import get_logger
from integritywatch.utils.platform.base import get_current_platform, is_windows
...
class NativeHostInstaller:
    ...
    def install(self) -> dict[str, bool]:
        ...
        print(f"Detected browsers: {', '.join(installed_browsers)}")
        ...
        for browser_name, manifest_dir in browser_paths.items():
            try:
                ...
                if success:
                    ...
                    print(f"{browser_name}: Registry key created")
                    results[browser_name] = True
                else:
                    self.logger.error(f"{browser_name}: Failed to create registry key")
                    print(f"{browser_name}: Failed to create registry key")
    ...
    def _install_unix(self, manifest_content: dict) -> dict[str, bool]:
        ...
        for browser_name, browser_path in browser_paths.items():
            try:
                ...
                print(f"{browser_name}: Installed to {manifest_file}")
                results[browser_name] = True
...
def main():
    ...
    try:
        ...
        if results and any(results.values()):
            print("Native host installation completed.")
            print("Browsers:")
            ...
            print("Installation failed. No browser was configured successfully.")
            sys.exit(1)
        ...
        print(f"Error: {e}")
        print("Please check the log file for full details.")
        sys.exit(1)
